...
#Defining functions and classes for non-daemonic pools
#-----
...
def get_default_executor():
    """
    Provide a default executor (a@wraps(MyPool)
        @contextmanager
        def Pool(*args, **kwargs):
            pool = MyPool(*args, **kwargs)
            yield pool
            pool.terminate()
        return Pool context manager
...
#        Pool = MyPool #I changed this one to use my non-daemonic pool
#        return Pool
...
def search(f, box, n, m, batch, resfile,num_workers,
    ...
    for i in range(n//batch):
        e = executor(num_workers)
        a = e.map(f, list(map(cubetobox, points[batch*i:batch*(i+1), 0:-1])))
                #print('map output:')
                #print(a)
        results = e.gather(a)
                #results = concurrent.futures.wait(a)
        #wait(results)
        points[batch*i:batch*(i+1), -1] = results
        if e.cluster.__class__.__name__ == 'SLURMCluster':
            e.cluster.stop_all_jobs()
        elif e.cluster.__class__.__name__ == 'LocalCluster':
            #stop gap solution. closing cluster seems to result in lots of errors so we dont do it for this case.
            #memory use will add up over time though
            pass
        else:
            e.close()
    ...
    for i in range(m//batch):
        ...
        e = executor(num_workers)
        a= e.map(f, list(map(cubetobox, points[n+batch*i:n+batch*(i+1), 0:-1])))
            #print('map output:')
            #print(a)
        results = e.gather(a)
        #wait(results)
        points[n+batch*i:n+batch*(i+1), -1] = list(np.array(results)/fmax)
        if e.cluster.__class__.__name__ == 'SLURMCluster':
            e.cluster.stop_all_jobs()
        elif e.cluster.__class__.__name__ == 'LocalCluster':
            #stop gap solution. closing cluster seems to result in lots of errors so we dont do it for this case.
    ...
    np.savetxt(resfile, points, delimiter=',', fmt=' %+1.11e', header=''.join(labels), comments='')
